...
OpenRGB is the only realistic source of broad RGB device support — 183
...
So the architecture is: **our service launches `OpenRGB.exe --server` as a
...
- **We don't touch any controller code**, ever. The 183 files in `Controllers/`
  are byte-identical to upstream, so upstream changes apply trivially.
...
The `Bundled/` directory is updated by downloading the Windows artifact from
the fork's CI run after each successful build.
